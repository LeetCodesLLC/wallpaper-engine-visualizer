...
(function($, createjs, document, undefined) {
    ...
    $.fn.visualizer = function(settings) {
        ...
        var generateFakeAudioEnabled = false;
        ...
        scope.generateFakeAudioData = function(enable) {
            generateFakeAudioEnabled = enable;
            /*if (enable) {
        ...
        var _sortVisuals = (a,b) => b.settings.zIndex - a.settings.zIndex;
        ...
        scope.updateVisuals = function(data) {
            ...
            let baseMultiplier = ARTHESIAN.Audio.Source.getBaseMultiplier(0, 5) - 1;
            baseMultiplier /= 5;
            ...
            _scale = baseMultiplier * scope.settings.bounceOnBeatStrengthMultiplier;
            
            _blur = baseMultiplier * 25;
        ...
        var _updateVisual = (visual) => {
            ...
            visual.update(_data);
                ...
                if (visual.gameObject) {
                    visual.gameObject.filters = [];
                    
                    if (visual.settings.bounceOnBeatEnabled) {
                        var bounceStrength = visual.settings.bounceStrength / 100 || 1;
                        visual.gameObject.scaleX = 1 + _scale * bounceStrength;
                        visual.gameObject.scaleY = 1 + _scale * bounceStrength;                        
                    } else {
                        visual.gameObject.scaleX = 1;
                        visual.gameObject.scaleY = 1;
                    }
                    
                    if (visual.settings.bassShakeEnabled) {
                        var bassStrength = visual.settings.bassShakeStrength / 100 || 1;
                        // Apply Shake to the container if applicable
                        if(visual.settings.bassShakeInvert) {
                            visual.gameObject.regX = -_offset.x * bassStrength;
                            visual.gameObject.regY = -_offset.y * bassStrength;
                        } else {
                            visual.gameObject.regX = _offset.x * bassStrength;
                            visual.gameObject.regY = _offset.y * bassStrength;
                        }
                    ...
                    if (visual.settings.blurOnBeatEnabled) {
                        var blurStrength = visual.settings.beatBlurStrength / 100 || 1;
                        var blurIntensity = _blur * blurStrength;
                        var blurFilter = new createjs.BlurFilter(blurIntensity, blurIntensity, 1);
                        visual.gameObject.filters.push(blurFilter);                        
                        var bounds = blurFilter.getBounds();
                        var gbounds = visual.gameObject.getBounds();
                        if (typeof bounds  != "undefined" && typeof gbounds != "undefined") {                            
                            visual.gameObject.cache(gbounds.x + bounds.x, gbounds.y + bounds.y, gbounds.width + bounds.width, gbounds.height + bounds.height);
                        }
                    }
        ...
        _this.update = function() {
            ...
            // Get Audio data
            if (!generateFakeAudioEnabled) {
                _newAudioData = ARTHESIAN.Audio.Source.getWallpaperEngineAudioData();
            } else {
                _newAudioData = _this.generateFakeAudioData();
            }
