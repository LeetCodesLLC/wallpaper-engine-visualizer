...
var visualisations = (function($, createjs, document, visualisations, undefined) {
    ...
    visualisations.background.confetti = function(id, settings) {
        ...
        scope.update = function(analyzer) {
            ...
            if (scope.gameObject.children.length > max) {
                while (scope.gameObject.children.length > max) {
                    if (!scope.gameObject.removeChildAt(0)) { break; };
                }
            }
        ...
        let _obj, _color, _travelDistance, _endScale, _animationSpeed, _movementSpeed; 
        _this.spawn = function() {
            ...
            _obj.regX += scope.settings.spawnCircleSize * 2;

            _travelDistance = window.innerHeight > window.innerHeight ? window.innerHeight : window.innerWidth;
            ...
            //createjs.Tween.get(_obj).to({ scaleX: 0.5, scaleY: 0.5 }, _animationSpeed);
            createjs.Tween.get(_obj).to({ scaleX : _endScale, scaleY: _endScale }, _animationSpeed, createjs.Ease.cubicOut).call(_removeConfettiFlake);
            createjs.Tween.get(_obj).to({ regX: _obj.targetX }, _movementSpeed);
